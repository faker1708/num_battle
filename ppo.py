...
class PolicyNet(nn.Module):
    def __init__(self, n_views, n_actions):
        # n_hiddens = 2**
        super(PolicyNet, self).__init__()

        # print('Ploci',n_views,n_actions)
        n_hiddens0 = 2**3
        n_hiddens1 = 2**3
        n_hiddens2 = 2**3
        # n_hiddens0 = 2**3
        ...
        self.in_put = nn.Linear(n_views, n_hiddens0)
    ...
    def forward(self, x):
        ...
        x = x.float()
        x = self.in_put(x)  # [b,n_views]-->[b,n_hiddens]
        x = F.relu(x)

        
        
        x = self.fc1(x)  # [b,n_views]-->[b,n_hiddens]
        x = F.relu(x)
        x = self.fc2(x)  # [b,n_views]-->[b,n_hiddens]
        x = F.relu(x)
...
class ValueNet(nn.Module):
    def __init__(self, n_views):
        ...
        n_hiddens = 2**3
        self.in_put = nn.Linear(n_views, n_hiddens)
...
class ppo:
    def __init__(self, n_views, n_actions,
        ...
        # 实例化策略网络
        self.actor = PolicyNet(n_views, n_actions).to(device)
        # 实例化价值网络
        self.critic = ValueNet(n_views).to(device)
    ...
    # 动作选择
    def take_action(self, view):
        # 维度变换 [n_view]-->tensor[1,n_views]
        view = torch.tensor(view[np.newaxis, :]).to(self.device)
        # 当前状态下，每个动作的概率分布 [1,n_views]
        probs = self.actor(view)
        # 创建以probs为标准的概率分布
        try:
    ...
    def learn(self, transition_dict):
        # 提取数据集
        views = torch.tensor(transition_dict['views'], dtype=torch.float).to(self.device)
        ...
        old_log_probs = torch.log(self.actor(views).gather(1, actions)).detach()

        # 一组数据训练 epochs 轮
        for _ in range(self.epochs):
            # 每一轮更新一次策略网络预测的状态
            log_probs = torch.log(self.actor(views).gather(1, actions))
            # 新旧策略之间的比例
            ...
            critic_loss = torch.mean(F.mse_loss(self.critic(views), td_target.detach()))
